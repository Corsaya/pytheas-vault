...
status: active plan — Phase 0 backup risk resolved 2026-08-26
related: ["[[01 — DONOVAN Master Context]]", "[[../../Roadmaps/Historical/Ultimate Workspace Roadmap]]", "[[../../Roadmaps/Development Roadmap]]"]
---
...
Chiron is already a fork of Odysseus (`~/code/chiron`, Docker, port 7001, five
current vaults ingested, generic Classroom interface). "Making it his own" is four distinct
...
**Resolved prerequisite — 2026-08-26:** private `Corsaya/chiron` now exists;
local `dev` tracks `origin/dev`. The SAT test/drill apps were subsequently
removed in commit `819892c`; the generic Classroom interface remains.

---

## 2. The architecture: one database, many faces

The mistake to avoid is building separate features that each own their own storage.
The thing that makes it *a workspace* rather than six apps is a single core.

```
                  ┌─────────────────────────────┐
                  │   Obsidian vaults (markdown) │   ← human-editable truth
                  │   5 vaults, per-vault git    │      for notes/journal/courses
                  └──────────────┬───────────────┘
                                 │  two-way sync
                  ┌──────────────▼───────────────┐
...
API, Notion Mail is scheduled to shut down 2026-09-22, and the data model is
closed. Ruled out
...
- ✅ Private GitHub remote for `~/code/chiron`; `dev` pushed to `Corsaya/chiron`.
- Same audit for `~/code/pytheas`, `~/code/jarvis`, and the five vault repos —
