...
|---|---|---|
| 1 | **Push `~/code/chiron` to a private GitHub remote** | Only `upstream` exists. 4+ commits including the whole Bluebook SAT app are local-only, one machine, no backup. **This is the single highest-risk item in the entire handoff.** |
| 2 | Verify `~/code/pytheas`, `~/code/jarvis`, and all 6 vault repos have live remotes and clean trees | Same failure mode, less material |
...
- **All six Obsidian vaults.** Markdown + git. Fully portable, which was the
...
| **last30days** (7-source research → daily briefings) | Standalone-ish; briefings are already generated into `pytheas/Briefings/`. **Worth checking whether it runs outside Claude Code.** If not, the daily-brief pipeline in `~/code/pytheas/briefing.py` is his own code and can call any API |
